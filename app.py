...
BOT_TOKEN = "8722342236:AAFAsRIctPewehU9B6Fb6plDCu1FtumUTp8"     # <-- PUT YOUR TOKEN HERE
# =========================================
...
def update_user_stats(user_id, is_valid, error_type=None):
    with user_lock:
        ...
        if ud["total_checked"] - ud["last_report"] >= 500:
            ...
            asyncio.create_task(send_message(user_id, report_text))
    ...
    while True:
        ...
        try:
            ...
            if status == 200:
                ...
                asyncio.create_task(send_message(user_id, f"🎉 *VALID CODE FOUND:* `{code}` 🎉"))
            else:
                mark_code_tried(code, is_valid=False)
                save_to_notvalid(code)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "First, send your mobile number (e.g., 9674662450).",
...
async def otp_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        send_otp(session, ud["mobile"], user_key, data_key)
...
async def run_bot():
    ...
    conv_handler = ConversationHandler(
        entry_points=[MessageHandler(filters.TEXT & filters.Regex("^/start$"), start)],
        states={
    ...
    application.add_handler(CallbackQueryHandler(button_callback, pattern="^(start_check|stop_check|stats|explain)$"))
